...
function handleConfirmation(handler) {
  if(handler.event.currentIntent.confirmationStatus == 'Confirmed') {
    // TODO: actually place the order here!
    handler.emit(':tell', "I've placed the order")
  } else {
    handler.emit(':tell', "Goodbye")
  }
}
...
var handlers = {
  'OrderCraftBeer.Dialog': function() {
    ...
    if(this.event.currentIntent.confirmationStatus && this.event.currentIntent.confirmationStatus != 'None') {
